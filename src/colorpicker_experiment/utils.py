"""Utility functions for the colorpicker experiment."""
...
def _orient(img: cv2.Mat) -> "npt.NDArray[np.int64]":
    ...
    return corners[largest].astype(int)
...
def _estimate_plates(
    orientation: "npt.NDArray[np.int64]", offset: dict
...
def _find_wells(
    plate_m: "npt.NDArray[np.float64]",
    ...
    pts = plate_m @ _to_homogeneous(pts).T
    ...
    return dict(zip(well_names, pts))


def _get_well_color(
    img: cv2.Mat, well: "npt.NDArray[np.int64]"
) -> "npt.NDArray[np.int64]":
    """Returns the color of a well in an image."""
...
def match_size(img: cv2.Mat, shape: tuple[int, int]) -> cv2.Mat:
    """
    Matches the size of an image to a given shape, cropping it if necessary.
    """
...
def get_colors(
    img: cv2.Mat, offset: Optional[dict] = None
) -> tuple[dict[int, dict[str, "npt.NDArray[np.int64]"]], cv2.Mat, cv2.Mat, cv2.Mat]:
    ...
    plates = _estimate_plates(orientation, offset)
    ...
    for plate_idx, plate in enumerate(plates):
        ...
        plate_m = _refine_plate(img, plate)
        if plate_m is None:
            raise RuntimeError(
                f"Could not refine plate {plate_idx + 1} with coordinates {plate}. "
                "Check if the plate is in the image and not mirrored."
            )

        # Find all of the well's pixel positions, and get the color there
        wells = _find_wells(plate_m)
        wells_img = img.copy()
        for pt in wells.values():
            x, y = pt
            color = [a.item() for a in img[y, x]]
            cv2.circle(wells_img, (x, y), int(plate_m[0, 0] / 2), color, -1)
        ...
        plates_d[plate_idx + 1] = wells

    if not plates_d:
        raise RuntimeError("Plate not detected at the expected location")

    return plates_d, plate_img, plate_only, plate_colors


def get_colors_from_file(
    img_path: PathLike, offset: Optional[dict] = None
) -> "npt.NDArray[np.int64]":
    """Analyzes an image file for plates and determines the colors of all wells."""

    img = cv2.imread(str(img_path))  # Load image
    if img is None:
        raise RuntimeError(f"Image {img_path} not loaded. Check file name.")

    img = match_size(img, (1280, 1920))  # Crop image
    plates_d, plate_img, plate_only, plate_colors = get_colors(img, offset=offset)
